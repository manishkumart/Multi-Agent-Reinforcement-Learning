...
class DataAugmentation:
    ...
    def shiftTimeSeries(self, tradingEnv, shiftMagnitude=0):
        ...
        if 'Volume' in newTradingEnv.data.columns:
            # shiftMagnitude = np.random.randint(-shift, shift)
            shiftMagnitude = np.random.randint(-10, 10)
            newTradingEnv.data['Volume'] += shiftMagnitude
        else:
            print("'Volume' column not found in the data. Skipping volume shift.")
