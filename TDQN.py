...
class TDQN:
    ...
    def __init__(self, observationSpace, actionSpace, numberOfNeurons=numberOfNeurons, dropout=dropout, 
        ...
        # Check availability of CUDA for the hardware (CPU or GPU or MPS)
        self.device = torch.device('cuda:'+str(GPUNumber) if torch.cuda.is_available() else ('mps' if torch.backends.mps.is_available() else 'cpu'))
